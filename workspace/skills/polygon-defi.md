...
> **Trails Integration**: Swaps, bridges, and earn pool discovery are all powered by [Trails](https://trails.build). The CLI uses `TRAILS_API_KEY` if set, otherwise falls back to the key generated by `polygon-agent setup` (stored in `~/.polygon-agent/builder.json`). For direct Widget, SDK, or API integration, load the `trails` skill. Get your API key at [https://dashboard.trails.build](https://dashboard.trails.build).
...
Set `TRAILS_API_KEY` to your Trails API key (from [https://dashboard.trails.build](https://dashboard.trails.build)), or omit it — the CLI falls back to the key from `polygon-agent setup`.
