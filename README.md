...
|--------|----------|-------------|
| `TRAILS_API_KEY` | Optional | Your Trails API key from [dashboard.trails.build](https://dashboard.trails.build). If set, takes precedence for all swap/deposit/bridge operations. If omitted, the CLI uses the key generated by `polygon-agent setup`. |
